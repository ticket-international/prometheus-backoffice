...
const BANNERS_API_URL = "https://ticket-cloud.com/wp-json/ti/v1/banners";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Fetching banners:', { blogId, page, perPage });

    const token = await generateTiToken(blogId);
    console.log('Generated token:', token);

    const apiUrl = `${BANNERS_API_URL}?blogId=${blogId}&page=${page}&perPage=${perPage}`;
    ...
    console.log('=== BANNERS EDGE FUNCTION RESPONSE ===');
    ...
    return new Response(
      JSON.stringify({ error: 'Failed to fetch banners', details: error.message }),
      {
        status: 500,
        headers: {
          ...corsHeaders,
