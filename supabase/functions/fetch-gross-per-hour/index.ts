...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const apiUrl = `https://api.cinster.ticketserver.net/3.0/reports/nexus/grossperhour?apikey=${apiKey}${siteParam}&from=${from}&to=${to}`;
    ...
    console.warn('Error fetching gross per hour:', error);
    return new Response(
      JSON.stringify({ data: [], total: 0 }),
      {
        status: 200,
